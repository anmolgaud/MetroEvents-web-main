...
const CreateEventRight = () => {
  ...
  const { ThemeLayout, TextColor, TypeFace } = useSelector(
    (store) => store.createEvent.themeOptions,
  );
  ...
  return (
    <div className="bg-white rounded-md p-4">
      {/* Canvas */}
      <div
        className={`flex justify-center items-center rounded-md p-6 ${Themes.get(
          ThemeLayout,
        )} bg-cover bg-center bg-no-repeat w-full h-[400px]`}
      >
        <div className="w-full">
          <textarea
            onChange={(e) => dispatch(changeThemeText(e.target.value))}
            className={`w-full bg-transparent ${
              Color.get(TextColor)[1]
            } ${fontFamily.get(
              TypeFace,
            )} text-3xl text-center p-4 focus:outline-white`}
      ...
      <div className="w-full mt-8 p-4 rounded-lg bg-neutral-100 flex flex-col gap-y-2 flex-nowrap">
        ...
        <div className="w-full flex justify-between items-center border-b-2 pb-2">
          <div className="flex items-center">
            <div
              className={`w-6 h-6 ${Color.get(TextColor)[0]} rounded-full`}
            ></div>
          ...
          <DropDown
            ...
            defaultValue={"white"}
            optionValues={["white", "black", "red", "indigo"]}
          />
        </div>
        <div className="w-full flex justify-between items-center">
          ...
          <DropDown
            ...
            optionValues={["Poppins", "Roboto", "Decorative"]}
          />
        </div>
      </div>
    </div>
